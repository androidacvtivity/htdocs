...
class Cl_caem{
    ...
    public function selectvw()
    {
        ...
        if($con != null)
        {
            $resultcu=$con->query(Constants::$SQL_SELECT_ALL_7);
            if($resultcu->num_rows > 0)
            {
                $cl_caem = array();
                while($row=$resultcu->fetch_array())
                {
                    array_push($cl_caem, array("id"=>$row['id'],"CODUL"=>$row['CODUL'],
					"DENUMIRE"=>$row['DENUMIRE'],"GRUPA"=>$row['GRUPA'],"ORDINE"=>$row['ORDINE']
					,"FULL_CODE"=>$row['FULL_CODE'],"NUM_CODE"=>$row['NUM_CODE']));
					
                }
                print(json_encode(array("codecu" => 1,"messagecu"=>"Success", "resultcu"=>$cl_caem)));
            }else{
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST')
        {
            if (isset($_POST['actioncu'])) {
                ...
                if($actioncu == 'INSERT'){
                    ...
                    $this->delete();
				}else if($actioncu == 'GET_PAGINATEDCAEM'){
                    $this->search_caem();
				}else if($actioncu == 'GET_PAGINATED_SEARCHCAEM'){
                    $this->search_caem();
                }else{
