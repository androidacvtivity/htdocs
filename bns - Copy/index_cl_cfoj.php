...
class Cl_cfoj{
    ...
    public function selectvw()
    {
        ...
        if($con != null)
        {
            $resultcu=$con->query(Constants::$SQL_SELECT_ALL_8);
            if($resultcu->num_rows > 0)
            {
                $cl_cfoj = array();
                while($row=$resultcu->fetch_array())
                {
                    array_push($cl_cfoj, array("id"=>$row['id'],"CODUL"=>$row['CODUL'],
					"DENUMIRE"=>$row['DENUMIRE'],"GRUPA"=>$row['GRUPA'],"ORDINE"=>$row['ORDINE'],"FULL_CODE"=>$row['FULL_CODE']));
					
                }
                print(json_encode(array("codecu" => 1,"messagecu"=>"Success", "resultcu"=>$cl_cfoj)));
            }else{
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST')
        {
            if (isset($_POST['actioncu'])) {
                ...
                if($actioncu == 'INSERT'){
                    ...
                    $this->delete();
				}else if($actioncu == 'GET_PAGINATEDCFOJ'){
                    $this->search_cl_cfoj();
				}else if($actioncu == 'GET_PAGINATED_SEARCHCFOJ'){
                    $this->search_cl_cfoj();
                }else{
