...
class Scientistscaem{
    ...
    public function selectvw()
    {
        ...
        if($con != null)
        {
            $resultcu=$con->query(Constants::$SQL_SELECT_ALL_3);
            if($resultcu->num_rows > 0)
            {
                $scientistscaem = array();
                while($row=$resultcu->fetch_array())
                {
                    array_push($scientistscaem, array("id"=>$row['id'],"DENUMIRE"=>$row['DENUMIRE'],
					"VALUE"=>$row['VALUE'],"act"=>$row['act']));
					
                }
                print(json_encode(array("codecu" => 1,"messagecu"=>"Success", "resultcu"=>$scientistscaem)));
            }else{
