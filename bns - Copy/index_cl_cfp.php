...
class Cl_cfp{
    ...
    public function selectvw()
    {
        ...
        if($con != null)
        {
            $resultcu=$con->query(Constants::$SQL_SELECT_ALL_10);
            if($resultcu->num_rows > 0)
            {
                $cl_cfp = array();
                while($row=$resultcu->fetch_array())
                {
                    array_push($cl_cfp, array("id"=>$row['id'],"CODUL"=>$row['CODUL'],
					"DENUMIRE"=>$row['DENUMIRE'],"GRUPA"=>$row['GRUPA'],"ORDINE"=>$row['ORDINE'],"FULL_CODE"=>$row['FULL_CODE']));
					
                }
                print(json_encode(array("codecu" => 1,"messagecu"=>"Success", "resultcu"=>$cl_cfp)));
            }else{
